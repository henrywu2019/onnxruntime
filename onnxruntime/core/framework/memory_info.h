...
class MemoryInfo {
 public:
  ...
  //(2) StaticActivation: Which are planned statically and allocated accordingly
  ...
  struct MemoryInfoProfile {
   public:
    ...
    void CreateEvents(const std::string& p_name,
                      const size_t pid,
                      const MemoryInfo::MapType& map_type,
                      const std::string& group_name,
                      const size_t top_k,
                      const OrtDevice::DeviceType device_t = OrtDevice::CPU);
  ...
  std::string GenerateMemoryProfile(const std::string& folder, const std::string& file_name);
