...
class block{
 ...
 public:
  block(int ih_, int ow_, int iw_, int co_, int Ci_):len(ow_),ih(ih_), iw(iw_), Co(co_), Ci(Ci_){
    pa = new float*[ih];
  }
  ...
  void run(int ch){
    int pa_base_idx = ch*ih*iw;
    for(int k=0;k<3;k++){
      // i == 0
      for (int j = 0; j < len; j++){
        auto z1 = *(pa[pa_base_idx] + j);
        for(int z=0;z< Co;z++){
          O[j] += z1 * F[k];
          O[j+z] += z1 * F[k+z*Ci*9+ch*9];
        }
      }
      // i == 1
      for (int j = 0; j < len; j++){
...
long long gme_conv_no_extraction(vector<float>& I,
                       ...
                       int Co, int input_h, int input_w, int output_h, int output_w) { // O(Ci*3*3*Co*(Oh*Ow))
  auto start = std::chrono::high_resolution_clock::now();
  block blk(input_h, output_w, input_w, Co, Ci);
  //for(int co=0;co<Co;co++){
    for(int channel=0;channel<Ci;channel++){
      //blk.reset(I.data()+channel*input_h*input_w,F.data()+channel*Kh*Kw+co*Kw*Kh*Ci, Output+co*output_h*output_w);
      blk.reset(I.data()+channel*input_h*input_w,F.data()+channel*Kh*Kw, Output);
      blk.run(channel);
    }
  //}
  auto t1 = std::chrono::high_resolution_clock::now();
