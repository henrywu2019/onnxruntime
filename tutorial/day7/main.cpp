...
void get_input(vector<float>& l, int n, int c, int w, int h) {
  if (l.size() < n * c * w * h) l.resize(n * c * w * h);
  float start = 1.;
  for (int i = 0; i < n; i++)
    for (int j = 0; j < c; j++) {
 ...
 * */
inline long long gme_conv(vector<float>& I,
              vector<float>& F,
              float* sliced_mat,
              float* Output,
              int Kh,
...
long long run(int input_height, int input_width, int input_channel, int filter_batch){
  const int kernel_width=3, kernel_height=3;
