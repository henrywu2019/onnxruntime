...
ax1.set_title("ZPZR vs. SOTA(Input=64x24x34,Kernel=3x3,AMD EPYC 7J13,2.2GHz,Cache 64/64/1.6),AVX2")
ax1.legend()

# Filter the data by f and K for the second subplot
t_f1 = df.loc[df['f'] == 1, 't']
K_f1 = df.loc[df['f'] == 1, 'K']
#t_f2 = df.loc[(df['f'] == 2) & (df['S']==64), 't']
#K_f2 = df.loc[(df['f'] == 2) & (df['S']==64), 'K']
t_f3 = df.loc[(df['f'] == 2) & (df['S']==32), 't']
K_f3 = df.loc[(df['f'] == 2) & (df['S']==32), 'K']
...
plt.savefig("zpzr_vs_sota.e3.24x34.64.png")
